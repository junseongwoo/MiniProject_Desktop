    ...
    public partial class EditUser : Page
    {
        ...
        private void Page_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // 그리드 바인딩
                List<Model.User> users = Logic.DataAccess.GetUesrs();
        ...
        public bool IsValidInput()
        {
            ...
            /*else
            {
                var cnt = Logic.DataAccess.GetUesrs()
                                .Where(u => u.UserIdentityNumber.Equals(TxtUserIdentityNumber.Text)).Count();
                if (cnt > 0)
                {
                    LblUserIdentityNumber.Visibility = Visibility.Visible;
            ...
            {
                var cnt = Logic.DataAccess.GetUesrs()
                                .Where(u => u.UserEmail.Equals(TxtUserEmail.Text)).Count();
                if (cnt > 0)
                {
                    LblUserEmail.Visibility = Visibility.Visible;
        ...
        private void BtnAdd_Click(object sender, RoutedEventArgs e)
        {
            ...
            //var user = Commons.LOGINED_USER; // 유저를 받아 오는것 
            //var user = new Model.User(); // 이렇게 하면 수정시 중복된 값을 넣음
            var user = GrdData.SelectedItem as Model.User; // 중복된것을 바꾸기 위해 이렇게 만듬
